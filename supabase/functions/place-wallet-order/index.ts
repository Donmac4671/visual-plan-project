...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const item of items) {
      console.log(`📦 Processing item: network_id=${item.network_id}`);
      ...
      // Check for Airtime or Mashup (exact match from frontend)
      ...
      if (isAirtimeOrMashup) {
        ...
        console.log(`✅ ${item.network_id} order ${orderIdString} completed (no GHData)`);
      } else {
        // Data bundle - send to GHData
        ...
        console.log(`📡 Data order ${orderIdString} sent to GHData`);
      }
    }
