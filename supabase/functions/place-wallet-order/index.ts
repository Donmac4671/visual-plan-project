...
serve(async (req) => {
  ...
  try {
    ...
    // SAFETY: Prevent Airtime/Mashup from going to GHData
    const NON_GH_PRODUCTS = ["airtime", "mashup", "airtime-mtn", "airtime-telecel", "airtime-at", "combo", "bundle"];
    if (NON_GH_PRODUCTS.includes(network_id.toLowerCase())) {
      console.log(`Non-GHData product detected: ${network_id}. Marking order as completed without calling GHData.`);

      await supabase
        .from("orders")
        .update({ status: "completed", gh_reference: `local-${Date.now()}` })
        .eq("id", order_id);

      return new Response(
        JSON.stringify({
          success: true,
          message: `${network_id} order completed locally (not sent to GHData)`,
